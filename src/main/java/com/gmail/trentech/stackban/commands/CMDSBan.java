	...
	public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
		...
		if(src.hasPermission("stackban.cmd.sban.add")) {
			list.add(Text.builder().color(TextColors.GREEN).onHover(TextActions.showText(Text.of("Click command for more information ")))
					.onClick(TextActions.executeCallback(Help.getHelp("add"))).append(Text.of(" /sban add")).build());
		}
		if(src.hasPermission("stackban.cmd.sban.remove")) {
			list.add(Text.builder().color(TextColors.GREEN).onHover(TextActions.showText(Text.of("Click command for more information ")))
					.onClick(TextActions.executeCallback(Help.getHelp("remove"))).append(Text.of(" /sban remove")).build());
		}
		if(src.hasPermission("stackban.cmd.sban.list")) {
			list.add(Text.builder().color(TextColors.GREEN).onHover(TextActions.showText(Text.of("Click command for more information ")))
					.onClick(TextActions.executeCallback(Help.getHelp("list"))).append(Text.of(" /sban list")).build());
		}
		if(src.hasPermission("stackban.cmd.sban.log")) {
			list.add(Text.builder().color(TextColors.GREEN).onHover(TextActions.showText(Text.of("Click command for more information ")))
					.onClick(TextActions.executeCallback(Help.getHelp("log"))).append(Text.of(" /sban log")).build());
		}
		if(src.hasPermission("stackban.cmd.whatsthis")) {
			list.add(Text.builder().color(TextColors.GREEN).onHover(TextActions.showText(Text.of("Click command for more information ")))
					.onClick(TextActions.executeCallback(Help.getHelp("whatsthis"))).append(Text.of(" /whatsthis")).build());
		}
		pages.contents(list);
